...
export default function ProtectedRoute({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (status === 'connected' && address) {
      const connectorId = config.state.connections.values().next().value?.connector?.id ?? '';
  ...
  useEffect(() => {
    ...
    const connector = connectorId
      ? config.connectors.find(c => c.id === connectorId) ?? config.connectors[0]
      : config.connectors[0];
    ...
    if (isOrangeKit) {
      connect({ connector, chainId: 31611 }, {
        onError: () => {
          // If silent reconnect fails, stop waiting — user needs to reconnect manually
          setWaiting(false);
        },
      });
    } else {
